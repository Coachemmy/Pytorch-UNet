...
# Directories for input data, masks, and saving checkpoints
dir_img = Path('./data/imgs/')
dir_mask = Path('./data/masks/')
dir_checkpoint = Path('./checkpoints/')

# Trains a U-Net model on the given dataset
def train_model(
        model,  # The U-Net model to train
        device,  # The device to use for training (GPU)
        epochs: int = 5,  # Number of training epochs
        batch_size: int = 1,  # Number of images in each training batch
        learning_rate: float = 1e-4,  # Learning rate for the optimizer
        val_percent: float = 0.1,  # Percentage of data used for validation
        save_checkpoint: bool = True,  # Flag to save checkpoints after each epoch
        img_scale: float = 1.0,  # Default scale set to 1.0 for better results
        amp: bool = False,  # Flag to enable Automatic Mixed Precision (AMP)
        weight_decay: float = 1e-8,  # Weight decay for the optimizer
        momentum: float = 0.999,  # Momentum for the RMSprop optimizer
        gradient_clipping: float = 1.0,  # Maximum gradient norm for clipping
):
    ...
    # STEP 4 - Setup optimizer, loss function, and learning rate scheduler
    ...
    for epoch in range(1, epochs + 1):
        ...
        if save_checkpoint:
            ...
            logging.info('Final model saved as MODEL.pth!')
